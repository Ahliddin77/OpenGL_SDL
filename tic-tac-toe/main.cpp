...
bool checkForWin()
{
    for (i = 0; i < n; ++i)
    {
        ...
        if (sumRow == oTotal || sumRow == crossTotal || sumColumn == oTotal || sumColumn == crossTotal)
        {
            return true;
        }
    }
    ...
    if (sumFirstDiagonal == oTotal || sumFirstDiagonal == crossTotal || sumSecondDiagonal == oTotal || sumSecondDiagonal == crossTotal)
    {
        return true;
    }
...
void display()
{
    ...
    for (i = 0; i < n; ++i)
    {
        for (j = 0; j < n; ++j)
        {
            ...
            switch (board[i][j])
            {
            case passive: glColor3ub(50, 50, 50); break;
            case active: glColor3ub(0, 150, 0); break;
            case crossPassive: glColor3ub(200, 0, 0); break;
            case crossActive: glColor3ub(255, 255, 0); break;
            case oPassive: glColor3ub(0, 0, 200); break;
            case oActive: glColor3ub(0, 150, 150); break;
            }
